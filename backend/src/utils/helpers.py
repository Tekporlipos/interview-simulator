...
def interview_prompt(index, data, interview_type="technical"):
    ...
    if index == 0:
        ...
        return (
            ...
            f'"type": "Question"}}. Simulate a {interview_type.split(" ")[0]} interview for a {data.get("position")} role'
            ...
            f'your Panels are {data.get("panels")}.'
            f'{" Based on these questions: " + question + "." if question is not None and len(question) > 5 else ""}'
            f'In any of your response make sure to include a {interview_type.split(" ")[0]} question and do not ask a '
            f'question that had been asked and answered. '
            f'If candidate answers incorrectly, provide brief feedback before moving on to the next question '
            f'do it in the same response. '
