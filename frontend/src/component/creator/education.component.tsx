...
import { IEducation } from "@/utlities/interfaces";

export default function EducationComponent(props: any) {
  const [education, setEducation] = useState<IEducation>({} as IEducation);

  function add(): void {
    props.setExperience([education, ...(props.experience ?? [])]);
    setEducation({
      program_name: "",
      institution_name: "",
      date_start: ["", ""],
  ...
  function edit(at: number): void {
    let iExperiences = props.experience.filter(
      (value: IEducation, index: number) => index !== at,
    );
    let find = props.experience.find(
      (value: IEducation, index: number) => index === at,
    );
    if (find) {
      setEducation(find);
      ...
      <div className="flex-column rounded my-2">
        ...
        <DateComponent
          setStartData={(data: string[]) =>
            setEducation({ ...education, date_start: data })
          }
          setEndData={(data: string[] | string) =>
            setEducation({ ...education, date_end: data })
          }
        ...
        <div className="mb-3 p-1 justify-content-start align-middle">
          <TextAreaComponent
            setintput={(data: string) =>
              setEducation({ ...education, description: data })
            }
            inputMessage={education.description}
            placeholder="Brief Description of Courses and Achievements"
          />
        </div>
      </div>
      {props?.experience?.map((data: IEducation, index: number) => (
        <ExperienceListComponent
          edit={edit}
          at={index}
          name={data.program_name}
          info={data.institution_name}
          key={index}
        />
      ))}
    </div>
