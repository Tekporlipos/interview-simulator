...
import { IPublication } from "@/utlities/interfaces";

export default function PublicationsComponent(props: any) {
  const [certification, setCertification] = useState<IPublication>(
    {} as IPublication,
  );
  ...
  function edit(at: number): void {
    let iExperiences = props.certifications.filter(
      (value: IPublication, index: number) => index !== at,
    );
  ...
  return (
    <div>
      <div className="flex-column rounded my-2">
        {certification.publication_name ? (
        ...
        <div className="mb-3 p-1 justify-content-start align-middle">
          <TextAreaComponent
            ...
            placeholder="Brief Description of Publication"
          />
        </div>
      </div>
      {props?.certifications?.map((data: IPublication, index: number) => (
